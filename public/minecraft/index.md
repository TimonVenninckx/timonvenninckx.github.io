...
At the end of week-8 I really wanted the fix add to add proper noise carver caves. It took me quite a some time to understand how to make such caves I ended up watching this GDC talk from [Squirrel Eiserloh](https://www.youtube.com/watch?v=Z8mAN378kIw). He has many good talks about RNG actually here is [playlist](https://www.youtube.com/watch?v=B1E0iVLg3NM&list=PLJbJTEvd4lbOH6hnh-6WcU__HpmrVxVOR&index=1) of some talks.
...
##### Bunch of items (Door is overriden)

![newicons](/images/minecraft/bunch_of_items.webp)

### Villages
They were quite a challenge, at first I was really pondering how I should implement this. As they have to be generated across chunk-borders and have to generated the same way in any order e.g. Northern chunk first then southern or the inverse. What I ended up going for was for every **20x20** chunks **region** I would decide if there was going to be a village or not. You could then check if your chunk falls within a **village region** and if It does we start generating the village roads first the **RNG** is seeded by the region X&Z-coordinates scrambled. First I generate some road going in the in both directions they can have variable lengths, I store their AABB's and then start looping over all the roads and at **pseudo-random** (the RNG is still based on the seed) locations next to the road I try to place houses by checking if their AABB overlaps with any roads or generated structure. Then after I have generated the roads and structure I check if **ANY** of them **START** in the current or neighbouring chunk. If they do I start generating the structure from their **origin** and if any of the blocks I'm trying to place fall into the currently generating **Chunk** I set the blocks.
```c++
...
```
##### Model
It's also possible to use a singular model instead of a block states file which points to model files. Here is an example of a crafting table which has the functionality added in behaviours, I could also ad this behaviour to another block and it would work the same.
```json
{
  "name": "Crafting Table",
  "type": "block",
